...
- *Priority filtering.*  Filter by Org priority, with a clickable priority
  cookie on each card.
- *Priority coloring.*  Cards reflect their Org priority using your configured
  ~org-priority-faces~ — by default the =[#X]= cookie is colored, or you can
  tint the whole card background (see ~org-kanban-modern-priority-style~).
...
- ~org-kanban-modern-priority-style~ — how a card reflects its Org priority
  ...
  priorities look consistent across Org.  Choices: ~cookie~ (tint the =[#X]=
  cookie — the default), ~background~ (tint the whole card background),
  ~both~, or ~nil~ (no priority coloring).
- ~org-kanban-modern-priority-tint~ — blend strength (0.0–1.0, default 0.18)
  of the priority color into the card background.  Only applies when
  ~org-kanban-modern-priority-style~ tints the background (~background~ or
  ~both~).
