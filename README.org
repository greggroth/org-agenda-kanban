...
- *Live data.*  Cards are collected from files or directories in
  ~org-kanban-modern-files~ (defaults to ~org-agenda-files~).
...
| =t p=                  | Filter by effective priority (prompt)       |
...
- ~org-kanban-modern-files~ — files or directories to scan (default
  ~org-agenda-files~).
...
- ~org-kanban-modern-header-remove-glyph~ /
  ~org-kanban-modern-header-done-window-prefix~ — text used in header filter
