...
Board edits use Org's normal commands and are saved immediately when the
source Org buffer was clean before the edit.  If that source buffer already
has unsaved edits, the board change is left unsaved too so unrelated edits are
not silently written to disk; save the Org buffer explicitly when ready.
