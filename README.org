...
- ~org-kanban-modern-scheduled-glyph~ / ~org-kanban-modern-deadline-glyph~ —
  ...
  (default ="⚑ "=) timestamps.  Prefer a monochrome (text-presentation)
  glyph so its width matches the card's fixed-pitch grid; avoid emoji such
  as =⏰=, which Emacs draws from a fallback emoji font whose advance does
  not align to the monospace grid (misaligning the timestamp).  Use a short
  text label (e.g. ="S: "=) if the glyph does not render in your font.
